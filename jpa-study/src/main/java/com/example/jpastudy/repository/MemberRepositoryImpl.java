...
public class MemberRepositoryImpl extends Querydsl5RepositorySupport implements MemberRepositoryCustom {
  ...
  public Slice<Member> findAllWithTeamSlice(MemberSearchConditionDTO condition, Pageable pageable) {
    return this.applySlicing(pageable, contentQuery -> contentQuery
        // content 쿼리만 있으면 된다.
        .selectFrom(member)
        .innerJoin(member.team, team).fetchJoin()
        .where(
